...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    if(stats->butthurt <= 4) {
        mood = 0;
        snprintf(mood_str, 20, "Humor: Feliz");
    } else if(stats->butthurt <= 9) {
        mood = 1;
        snprintf(mood_str, 20, "Humor: Ok");
    } else {
        mood = 2;
        snprintf(mood_str, 20, "Humor: Zangado");
    }
    ...
    snprintf(level_str, 20, "Nivel: %hu", stats->level);
    canvas_draw_str(canvas, 58, 12, my_name ? my_name : "Desconhecido");
