...
void bt_settings_scene_forget_dev_confirm_on_enter(void* context) {
    ...
    dialog_ex_set_header(dialog, "Desparear Todos Disp.?", 64, 3, AlignCenter, AlignTop);
    dialog_ex_set_text(
        dialog, "Todos os pareamentos\nanteriores serao perdidos!", 64, 22, AlignCenter, AlignTop);
    dialog_ex_set_left_button_text(dialog, "Voltar");
    dialog_ex_set_right_button_text(dialog, "Desparear");
