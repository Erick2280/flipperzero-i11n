...
static NotificationAppSettings* alloc_settings() {
    ...
    item = variable_item_list_add(
        app->variable_item_list, "Luz LCD", BACKLIGHT_COUNT, backlight_changed, app);
    ...
    item = variable_item_list_add(
        app->variable_item_list, "Tempo da luz", DELAY_COUNT, screen_changed, app);
    ...
    item = variable_item_list_add(
        app->variable_item_list, "Brilho do LED", BACKLIGHT_COUNT, led_changed, app);
    value_index = value_index_float(
    ...
    if(furi_hal_rtc_is_flag_set(FuriHalRtcFlagStealthMode)) {
        ...
        value_index = 0;
        variable_item_set_current_value_index(item, value_index);
        variable_item_set_current_value_text(item, "Furtivo");
    } else {
        item = variable_item_list_add(
    ...
    if(furi_hal_rtc_is_flag_set(FuriHalRtcFlagStealthMode)) {
        ...
        item = variable_item_list_add(
            app->variable_item_list, "Vibracao", VIBRO_COUNT, vibro_changed, app);
        value_index =
